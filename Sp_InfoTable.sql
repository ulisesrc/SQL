...
BEGIN
	...
	SET @_Script = N'DECLARE @_Campo CHAR(2)          
			 ...
			 SET @_Campo = ' + '''' + '''' + '
				        SELECT Tabla,         Columna,      Tipo,       [Tamaño],   Es_Identity, 
					       Permite_Nulos, ValorDefault, PrimaryKey, ForeignKey, Descripcion
					  FROM (SELECT Tabla,         Columna,     Tipo,        [Tamaño], 
						       Permite_Nulos, Es_Identity, Descripcion, ValorDefault, 
						       CASE WHEN PrimaryKey = 1 THEN ' + '''' + '1' + '''' + ' ELSE ' + '''''' + ' END AS PrimaryKey, 
						       CASE WHEN ForeignKey = 1 THEN ' + '''' + '1' + '''' + ' ELSE ' + '''''' + ' END AS ForeignKey,
						       CASE WHEN ' + CAST(@OrderBy AS VARCHAR) + ' = 1 THEN 
						                 (SELECT Ordinal_Position FROM ' + @_BaseDatos + '.INFORMATION_SCHEMA.COLUMNS C2 WHERE Tb2.Tabla = C2.TABLE_NAME AND Tb2.Columna = C2.COLUMN_NAME)
							    ELSE 0 
						       END AS OrderBY
					         FROM (SELECT Tabla,    Columna,       Tipo, 
						              [Tamaño], Permite_Nulos, Es_Identity, ValorDefault,
						         ...
						         FROM (SELECT C.TABLE_NAME       AS Tabla, 
								      ...
								      CASE WHEN ISNULL(C.CHARACTER_MAXIMUM_LENGTH, 0) > 0 THEN 
									   ...
									   WHEN C.DATA_TYPE = ' + '''' + 'NUMERIC' + '''' + ' OR C.DATA_TYPE = ' + '''' + 'DECIMAL' + '''' + ' THEN ' +
								      ...
								      CASE WHEN EXISTS(SELECT 1 FROM ' + @_BaseDatos + '.SYS.COLUMNS Col WHERE OBJECT_ID(' + '''' + @_BaseDatos + '''' + ' + ' + '''' + '.' + '''' + ' + C.TABLE_SCHEMA + ' + '''' + '.' + '''' + ' + C.TABLE_NAME) = Col.OBJECT_ID AND Col.Is_Identity = 1 AND C.Column_Name = Col.Name) THEN ' + '''' + '1' + '''' + ' ELSE ' + '''''' + ' END AS Es_Identity									 
								...
								WHERE (C.Table_Name  LIKE @TableName  OR  @TableName  = @_Campo) 
								  ...
								  AND (     @Constraint = 0 OR (@Constraint   = 1 AND  LEFT(K.CONSTRAINT_NAME, 2) = ' + '''' + 'PK' + '''' + ')
					               ...
					               GROUP BY Tabla, Columna, Tipo, [Tamaño], Permite_Nulos, Es_Identity, ValorDefault
					             ) Tb2
				              ) Tb3
